<template>
  <div class="main">
    <div class="container">
      <div class="wrapper d-flex m-3">
        <h2 v-if="student.name">Welcome to your Dashboard, {{ student.name }}</h2>
        <h2 v-else>Loading...</h2>
        <img src="../assets/student.gif" alt="">
      </div>

      <div class="row justify-content-center">
        <div class="col-md-3 col-sm-6 justify-content-center">
    <a href="#" class="card education" @click="attemptQuiz(quiz)">
      ...
      <p>Attempt Quiz</p>
        ...
        <div class="col-md-3 col-sm-6 justify-content-center">
          <a href="#" class="card credentialing" @click="showComponent('quizResults')">
            <div class="overlay"></div>
            <div class="circle">
        ...
        <div class="col-md-3 col-sm-6 justify-content-center">
          <a href="#" class="card wallet" @click="showComponent('studentDashboard')">
            <div class="overlay"></div>
            <div class="circle">
        ...
        <div class="col-md-3 col-sm-6 justify-content-center">
          ...
          <a href="#" class="card human-resources" @click="showComponent('personalDetails')">
            <div class="overlay"></div>
            <div class="circle">
        ...
        <QuizAttempt v-if="currentComponent === 'quizAttempt'" :quiz="currentQuiz" />
      <PersonalDetailsForm v-if="currentComponent === 'personalDetails'" />
      <StudentDashboard v-if="currentComponent === 'studentDashboard'" :quizzes="quizzes" />
      <QuizResults v-if="currentComponent === 'quizResults'" :results="quizResults" />
...
import QuizAttempt from "./QuizAttempt.vue";
...
export default {
  ...
  data() {
    return {
      student: {}, // Holds student details
      assignedQuizzes: [],
      pendingQuizzes: [],
      attemptedQuizzes: [],
      currentComponent: null, // Keep track of the active component
      currentQuiz: null, // Hold the current quiz data
    };
  },
  mounted() {
    this.fetchStudentData(); // Fetch student data when the component is mounted
  },
  methods: {
    fetchStudentData() {
      try {
        ...
        if (studentData) {
          ...
          // Assuming quizzes are also stored in localStorage
          // const quizzes = JSON.parse(localStorage.getItem('quizzes'));
          // if (quizzes) {
          //   this.filterQuizzes(quizzes);
          // } else {
          //   console.error("No quizzes found in local storage.");
          // }
        } else {
    ...
    filterQuizzes(quizzes) {
      ...
      // this.assignedQuizzes = quizzes.assigned.map(id => this.fetchQuizById(id));
      // this.pendingQuizzes = quizzes.pending.map(id => this.fetchQuizById(id));
      this.attemptedQuizzes = quizzes.attempted.map(id => this.fetchQuizById(id));
    },
    attemptQuiz(quiz) {
      // this.currentQuiz = quiz; // Set the current quiz
      this.currentComponent = 'quizAttempt'; // Show the QuizAttempt component
    }
  }
};
</script>
